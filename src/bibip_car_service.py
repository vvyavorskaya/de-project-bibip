...
class CarService:
    ...
    def sell_car(self, vin: str, buyer_info: Optional[Dict[str, Any]] = None, sale_price: float = 0.0) -> Sale:
        ...
        if car.status == CarStatus.sold:
            raise ValueError(f"Автомобиль с VIN {vin} уже продан.")

        car.status = CarStatus.sold
        self._update_car_status_or_data(car)

        sales_number = f"{datetime.now().strftime('%Y%m%d')}#{vin}"
        sale_date_obj = datetime.now()

        new_sale = Sale(
            sales_number=sales_number,
            car_vin=vin,
            sales_date=sale_date_obj,
            cost=Decimal(str(sale_price)))
        
        self._write_record_and_update_index(
            self.sales_file, 
            self.sales_index_file, 
            new_sale.sales_number, 
            self._sale_to_string(new_sale))
    ...
    def update_vin(self, vin: str, new_vin: str) -> Car:
        ...
        if not car:
            raise ValueError(f"Автомобиль с VIN {vin} не найден.")
        if self._get_car_by_vin(new_vin):
            raise ValueError(f"Автомобиль с VIN {new_vin} уже существует.")

        old_car_obj = car 
        new_car_obj = Car(
            vin=new_vin, 
            model=old_car_obj.model, 
            price=old_car_obj.price, 
            date_start=old_car_obj.date_start, 
            status=old_car_obj.status
        )

        self._remove_record_from_file(self.cars_file, vin) 
        self._write_record_and_update_index(self.cars_file, self.cars_index_file, new_car_obj.vin, self._car_to_string(new_car_obj))

        self._update_references_in_sales(vin, new_vin) 

        return self._find_car_by_vin(new_vin)
